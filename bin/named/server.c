...
configure_peer(const cfg_obj_t *cpeer, isc_mem_t *mctx, dns_peer_t **peerp) {
	...
	if (obj != NULL) {
		...
		// OQS changed from 4096 to 20480
		if (udpsize > 20480U) {
			udpsize = 20480U;
		}
...
#define CAP_IF_NOT_ZERO(v, min, max)        \
	...
	// OQS updated from 4096 to 20480
	CAP_IF_NOT_ZERO(recv_tcp_buffer_size, 20480, INT32_MAX);
	...
	// OQS updated from 4096 to 20480
	CAP_IF_NOT_ZERO(send_tcp_buffer_size, 20480, INT32_MAX);
	...
	// OQS updated from 4096 to 20480
	CAP_IF_NOT_ZERO(recv_udp_buffer_size, 20480, INT32_MAX);
	...
	// OQS updated from 4096 to 20480
	CAP_IF_NOT_ZERO(send_udp_buffer_size, 20480, INT32_MAX);
...
named_add_reserved_dispatch(named_server_t *server,
	...
	result = dns_dispatch_getudp(named_g_dispatchmgr, named_g_socketmgr,
				     // OQS updated from 4096 to 20480
				     named_g_taskmgr, &dispatch->addr, 20480,
